#!/bin/sh
#
# /etc/init.d/tizbin  --  start/stop the tizbin daemon. tizbin will decode the capture you upload to its web interface (default http://localhost:9876)
# 
### BEGIN INIT INFO
# Provides:          tizbin
...
# Default-Stop:      0 1 6
# Short-Description: Starts and stops the tizbin daemon
### END INIT INFO

PATH=/bin:/usr/bin:/sbin:/usr/sbin:/opt/tizbin/bin

NAME="tizbin"
DAEMON=/opt/tizbin/bin/dema
#APPLICATION=/opt/tizbin/bin/dema
...
#INTERFACE=eth1
#CONFIG_FILE="/opt/tizbin/cfg/tizbin.cfg"
#MODE=pcap
#PATH_DECODED_FILES="/opt/tizbin/bin/"
...
do_start()
{
	...
	case "$?" in
		0) echo "tizbin was (and is) already running" ;; 
		1) # ulimit
		   ulimit -n 200000
		   ulimit -m unlimited
		   ...
		   #killall dema
		   # start dema
		  (cd /opt/tizbin/bin; ./dema -d /opt/tizbin -b sqlite > /dev/null) &
		  #Optional: let's give more priotity and CPU to tizbin, so decoding will be faster.
		  ...
		  if [ "$PRIORITY" -ge "0" ] && [ "$PRIORITY" -le "20" ]; then
		    ...
		    echo "WARNING: tizbin priority not altered: wrong priority value (check $PRIORITY, range 0..20, default:0)"
		  fi
		  ;;
...
case "$1" in
	...
	echo
        ;;
    stop)
       # if [ -f /var/run/tizbin.pid ] && kill -0 `cat /var/run/tizbin.pid` 2>/dev/null; then
    ...
    restart|force-reload)
		...
		status)
			if is_running; then
				log_success_msg "tizbin web interface IS RUNNING to decode traffic capture files"
				exit 0
			else
				log_failure_msg "tizbin web interface mode is not running."
				exit 1
			fi
			;;				
    *)
        echo "Usage: /etc/init.d/tizbin {start|stop|restart|force-reload|status}"
        exit 1
        ;;
esac
