...
int SeDeInit(char *cert, char *root_dir)
{
    ...
    if (cert[0] == '\0') {
        /* default file path */
        sprintf(cert, DM_DEFAULT_TIZBIN_CERT, root_dir);
    }
    if (stat(cert, &info) != 0) {
        ssl_cert = NULL;
        return 0;
...
int SeDeStart(dbconf *db_c, char *main_dir, int pol, int session, task *pid, bool rt, char *interf, char *filter)
{
    ...
    case DB_POSTGRESQL:
        xpl_cfg = DM_TIZBIN_POSTGRES_CFG;
        for (i=0; i!=n; i++) {
    ...
    if (ptsk == 0) {
        ...
        for (i=0; i!=n; i++) {
            if (manip[i].enabled == TRUE) {
                sprintf(cmd, "echo "DM_TIZBIN_MANIP" >> %s", manip[i].dsctr, manip[i].port, config_file);
                system(cmd);
            }
        }
        ...
        system(cmd);
        sprintf(cmd, "echo TMP_DIR_PATH="DM_TMP_DIR"/tizbin >> %s", main_dir, pol, config_file);
        system(cmd);
        ...
        /* tizbin process */
        sprintf(app_path, "%s/bin/tizbin", main_dir);
        sprintf(work_dir, DM_DECOD_DIR, main_dir, pol, session);
        if (rt == FALSE) {
            if (stat(bpf_file, &sbuf) == 0) {
                execlp(app_path, "tizbin", "-c", config_file, "-m", "pol", "-d", work_dir,
                       "-F", bpf_file, NULL);
            }
            else {
                execlp(app_path, "tizbin", "-c", config_file, "-m", "pol", "-d", work_dir, NULL);
            }
        }
        else {
            if (filter != NULL && filter[0] != '\0') {
                execlp(app_path, "tizbin", "-c", config_file, "-m", "rltm_pol", "-r", "-i", interf, "-f", filter, "-d", work_dir, NULL);
            }
            else {
                execlp(app_path, "tizbin", "-c", config_file, "-m", "rltm_pol", "-r", "-i", interf, "-d", work_dir, NULL);
            }
        }
        exit(-1);
    }
...
int SeDeKill(podec *tbl, int id)
{
    ...
    if (tbl[id].run == TRUE) {
        n = sizeof(manip)/sizeof(manipula);
        /* decoder */
        if (tbl[id].pid.tizbin != 0)
            kill(tbl[id].pid.tizbin, SIGKILL);
        /* manipulators */
        for (i=0; i!=n; i++) {
