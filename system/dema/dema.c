...
static int DemaLoop(dbconf *db_c, char *root, time_t twpcap)
{
    ...
    while (1) {
        ...
        if (terminate == TRUE) {
            ...
            system("killall tizbin 2>/dev/null 1>/dev/null");
                    ...
                    if (pd_tbl[i].run == TRUE) {
                        if (SeDeRun(&pd_tbl[i].pid, chld, TRUE) == 0) {
                            ...
                            if (pd_tbl[i].end && filename == NULL) {
                                ...
                                SeDeKill(pd_tbl, i);
                                LogPrintf(LV_ERROR, "tizbin or a Manipulator is dead!\n");
                            }
                            if (pd_tbl[i].pid.tot == 0) {
                                ...
                                /* if there is a pcap then there is a fault.
                                   ...
                                   can do to crash tizbin */
                                if (filename != NULL) {
                                    LogPrintf(LV_ERROR, "tizbin or a Manipulator is dead! See file: %s in the fault dir\n", filename);
