...
def usage(argv):
   ...
   <main_dir>: main dir, by default /opt/tizbin
   ...
   For every protocol there is a subdirectory in the session  directory (/opt/tizbin/pol_YY/sol_XX), you can add new directory for your modules, by editing the file /opt/tizbin/cfg/sol_subdir.cfg, this give you the possibility to use Lucene-Solr even with your data.
""")
    ...
    for o, a in opts:
        ...
        if o == "-b":
            if a.isdigit():
                database = int(a)
            if database > 1:
                print("To use PostgreSQL contact: info@tizbin.net")
                sys.exit(2)
        # password
        if o == "-p":
            dbpassword = a
