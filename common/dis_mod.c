...
static void SegFault(int sig)
{
    ...
    if (fid != -1) {
        ...
        LogPrintf(LV_ERROR, "Thread without tizbin's protol-stack (possible main flow)");
    }
    /* save to file in wireshark filter format all flows active in the time of fault */
    LogFault("Segmentation Fault");
    printf("Segmentation Fault: see log file and report it to the developers: bug@tizbin.org\n");
    fflush(NULL);
#if 0
    while (1)
        sleep(1);
