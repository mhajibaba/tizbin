...
#define TIZBIN_BUFFER_MAX_SIZE    1024
...
int main(int argc, char *argv[])
{
    ...
    int c, i;
    char config_file[TIZBIN_BUFFER_MAX_SIZE];
    char module_name[TIZBIN_BUFFER_MAX_SIZE];
    char info_prot[TIZBIN_BUFFER_MAX_SIZE];
    ...
    while (capt == FALSE && (c = getopt(argc, argv, "vc:hi:glsm:")) != -1) {
        switch (c) {
        case 'v':
            printf("tizbin %d.%d.%d\n", TIZBIN_VER_MAG, TIZBIN_VER_MIN, TIZBIN_VER_REV);
            version = TRUE;
            break;

        case 'c':
            snprintf(config_file, TIZBIN_BUFFER_MAX_SIZE, "%s", optarg);
            config_file[TIZBIN_BUFFER_MAX_SIZE-1] = '\0';
            cfg_f = TRUE;
            break;
        ...
        case 'i':
            info = TRUE;
            snprintf(info_prot, TIZBIN_BUFFER_MAX_SIZE, "%s", optarg);
            info_prot[TIZBIN_BUFFER_MAX_SIZE-1] = '\0';
            break;

        case 'm':
            capt = TRUE;
            snprintf(module_name, TIZBIN_BUFFER_MAX_SIZE, "%s", optarg);
            module_name[TIZBIN_BUFFER_MAX_SIZE-1] = '\0';
            break;
    ...
    if (LogDirName(config_file) == -1) {
        if (cfg_f == FALSE) {
            ...
            strcpy(config_file, "/opt/tizbin/cfg/tizbin_cli.cfg"); /* next default */
            if (LogDirName(config_file) == -1) {
